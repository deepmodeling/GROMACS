...
/*!
 * \brief Implement MultiProcessingResources for the client MPI implementation.
 *
...
class MultiProcessingResourcesImpl : public MultiProcessingResources
{
public:
    ~MultiProcessingResourcesImpl() override = default;

    /*!
     * \brief Initialize from borrowed communicator.
     *
     * \param communicator Client-provided communicator relayed through lendCommunicator()
     */
    explicit MultiProcessingResourcesImpl(const CommT& communicator) : communicator_{ communicator }
    {
 ...
 *
 * \see createContext(std::unique_ptr<MultiProcessingResources> resources)
 *
 * The communicator is lent by the client. Client remains responsible for freeing
 * the communicator and finalizing the MPI environment.
 *
...
std::unique_ptr<MultiProcessingResources> lendCommunicator(CommT communicator)
{
    ...
    return std::make_unique<MultiProcessingResourcesImpl<CommT>>(communicator);
}
