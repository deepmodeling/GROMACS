...
def get_hipsycl(args):
    ...
    cmake_opts = [f'-DLLVM_DIR=/usr/lib/llvm-{llvm_version}/cmake',
                  f'-DCLANG_EXECUTABLE_PATH=/usr/bin/clang++-{llvm_version}',
    ...
    if args.cuda is not None:
        postinstall += [
            ...
            # Not needed unless we're building with CUDA-11, but no harm in doing always
            f'ln -s /usr/local/cuda-{args.cuda}/compat/* /usr/local/cuda-{args.cuda}/lib64/'
            ]
...
def build_stages(args) -> typing.Iterable[hpccm.Stage]:
    ...
    if args.cuda is not None and args.llvm is not None:
        ...
        building_blocks['cuda-clang-workaround'] = hpccm.primitives.shell(commands=[
            f'echo "CUDA Version {args.cuda}.0" > /usr/local/cuda-{args.cuda}/version.txt'
            ])
