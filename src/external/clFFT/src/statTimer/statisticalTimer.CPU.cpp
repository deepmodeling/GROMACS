...
void CpuStatTimer::setNormalize(bool norm)
{
    normalize = norm;
}
...
cl_double CpuStatTimer::getMean(size_t id) const
{
    ...
    Accumulator<cl_ulong> sum =
            std::for_each(clkTicks.at(id).begin(), clkTicks.at(id).end(), Accumulator<cl_ulong>());
...
std::ostream& operator<<(std::ostream& os, const CpuStatTimer& st)
{
    ...
    {
        cl_ulong                                min = 0;
        CpuStatTimer::clkVector::const_iterator iter =
