...
void GpuStatTimer::Stop(size_t id)
{
    ++currRecord;
}

void GpuStatTimer::AddSample(clfftPlanHandle            plHandle,
                             FFTPlan*                   plan,
                             cl_kernel                  kern,
                             cl_uint                    numEvents,
                             cl_event*                  ev,
                             ...
                             const std::vector<size_t>& lWorkSize)
{
    ...
    {
        timerData.at(currID).at(currSample).push_back(StatData(plHandle, plan, kern, numEvents, ev, gWorkSize, lWorkSize));
        ++currSample;
    }
}
...
void GpuStatTimer::queryOpenCL(size_t id)
{
    for (size_t s = 0; s < timerData.at(id).size(); ++s)
    {
        for (size_t n = 0; n < timerData.at(id).at(s).size(); ++n)
        {
            ...
            for (size_t i = 0; i < sd.outEvents.size(); ++i)
            {
                if (::clGetEventProfilingInfo(sd.outEvents[i], CL_PROFILING_COMMAND_START,
                                              sizeof(cl_ulong), &profStart, NULL)
                    != CL_SUCCESS)
                {
                    profStart = 0;
                }
...
std::vector<StatData> GpuStatTimer::getMean(size_t id)
{
    ...
    for (size_t s = 0; s < timerData.at(id).size(); ++s)
    {
        Accumulator<StatData> sum = std::for_each(
                timerData.at(id).at(s).begin(), timerData.at(id).at(s).end(), Accumulator<StatData>());
...
std::vector<StatData> GpuStatTimer::getVariance(size_t id)
{
    ...
    for (cl_uint v = 0; v < variance.size(); ++v)
    {
        ...
        {
            cl_double diff = static_cast<cl_double>(timerData[id][v][n].deltaNanoSec)
...
std::vector<StatData> GpuStatTimer::getMinimumTime(size_t id)
{
    ...
    for (size_t s = 0; s < timerData.at(id).size(); ++s)
    {
        ...
        {
            iter->doubleNanoSec =
                    static_cast<cl_double>(iter->deltaNanoSec) / timerData.at(id).at(s).size();
...
std::vector<size_t> GpuStatTimer::pruneOutliers(size_t id, cl_double multiple)
{
    ...
    for (size_t s = 0; s < timerData.at(id).size(); ++s)
    {
        ...
        StatDataVec::iterator newEnd = std::remove_if(
    ...
    for (cl_uint l = 0; l < labelID.size(); ++l)
    {
        ...
        for (cl_uint c = 0; c < lCount.size(); ++c)
        {
            std::cout << labelID[l].first << "[ " << c << " ]"
                      << ": Pruning " << lCount[c] << " samples out of " << currRecord << std::endl;
            tCount += lCount[c];
        }
    }
...
void GpuStatTimer::Print()
{
    ...
    for (cl_uint id = 0; id < labelID.size(); ++id)
    {
        ...
        //	Print label of timer, in a header
        std::cout << std::endl
                  << std::setw(tableHalf + halfString) << std::setfill('=') << labelID[id].first
        ...
        for (cl_uint t = 0; t < mean.size(); ++t)
        {
            ...
            if (mean[t].kernel == NULL)
            {
                ...
                {
                    if (mean[m].plHandle == mean[t].planX || mean[m].plHandle == mean[t].planY
                        || mean[m].plHandle == mean[t].planZ || mean[m].plHandle == mean[t].planTX
                        || mean[m].plHandle == mean[t].planTY || mean[m].plHandle == mean[t].planTZ
                        || mean[m].plHandle == mean[t].planRCcopy || mean[m].plHandle == mean[t].planCopy)
                    {
            ...
            if (mean[t].kernel != 0)
            {
                tout << std::setw(tableFourth) << _T( "Kernel:" ) << std::setw(tableThird)
            ...
            {
                tout << std::setw(tableFourth) << _T( "Batch:" ) << std::setw(tableThird)
                     << mean[t].batchSize << std::endl;
            }

            tout << std::setw(tableFourth) << _T("Placeness:") << std::setw(tableThird)
                 << (mean[t].placeness == CLFFT_INPLACE ? "InPlace" : "OutOfPlace") << std::endl;

            tout << std::setw(tableFourth) << _T("Input Dist:") << std::setw(tableThird)
                 << mean[t].iDist << std::endl;
            tout << std::setw(tableFourth) << _T("Output Dist:") << std::setw(tableThird)
                 << mean[t].oDist << std::endl;
            ...
            tout << std::setw(tableFourth) << _T( "Gflops:" ) << std::setw(2 * tableFourth)
                 << gFlops << std::endl;
            tout << std::setw(tableFourth) << _T( "Time (ns):" ) << std::setw(3 * tableFourth)
