 ...
 */
#ifndef GMX_COORDINATEIO_FRAMECONVERTERS_SHIFTCOORD_H
#define GMX_COORDINATEIO_FRAMECONVERTERS_SHIFTCOORD_H
...
public:
     ...
     * framework.
     */
    explicit ShiftCoord(const RVec shift, const Selection& sel) : shift_(shift), sel_(sel) {}
    /*! \brief
     *  Move constructor for ShiftCoord.
     */
    ShiftCoord(ShiftCoord&& old) noexcept : shift_(old.shift_), sel_(old.sel_) {}
    ...
    {
        return convertFlag(FrameConverterFlags::NoGuarantee);
    }
...
using ShiftCoordPointer = std::unique_ptr<ShiftCoord>;
