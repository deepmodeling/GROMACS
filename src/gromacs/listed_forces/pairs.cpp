                                        ...
                                        real*      dvdl)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        if ((qq[i] != 0) || (c6[i] != 0) || (c12[i] != 0))
        {
            ...
            {
	              rQ = gmx::sixthroot(1.-LFC[i]) * (1. + fabs(qq[i]/facel));
                rQ *= alpha_coul_eff;
            }
            ...
            if ((softcoreType == SoftcoreType::Gapsys) && (r<rQ))
            {
                ...
                dvdl_elec[i] += DLF[i] * 0.5 * (LFC[i] / (1. - LFC[i])) * (quadrFac - 2. * linFac + constFac);
            }
    ...
    dvdl[efptCOUL] += dvdl_coul_sum;
    dvdl[efptVDW] += dvdl_vdw_sum;
                             ...
                             int*                global_atom_index)
{
    ...
    for (i = 0; (i < nbonds);)
    {
        ...
        if (bFreeEnergy)
        {
            ...
            {
                fscal = free_energy_evaluate_single<SoftcoreType::Gapsys>(
                        r2, *fr->ic->softCoreParameters, fr->pairsTable->scale,
                        fr->pairsTable->data.data(), fr->pairsTable->stride, qq, c6, c12, qqB, c6B, c12B, epsfac,
            ...
            {
                fscal = free_energy_evaluate_single<SoftcoreType::Beutler>(
                        r2, *fr->ic->softCoreParameters, fr->pairsTable->scale,
                        fr->pairsTable->data.data(), fr->pairsTable->stride, qq, c6, c12, qqB, c6B, c12B, epsfac,
                        LFC, LFV, DLF, lfac_coul, lfac_vdw, dlfac_coul, dlfac_vdw, &velec, &vvdw, dvdl);
            }
            ...
            {
                fscal = free_energy_evaluate_single<SoftcoreType::None>(
                        r2, *fr->ic->softCoreParameters, fr->pairsTable->scale,
                        fr->pairsTable->data.data(), fr->pairsTable->stride, qq, c6, c12, qqB, c6B, c12B, epsfac,
                        LFC, LFV, DLF, lfac_coul, lfac_vdw, dlfac_coul, dlfac_vdw, &velec, &vvdw, dvdl);
            }
