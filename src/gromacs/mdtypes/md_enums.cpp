...
const char* evdw_names[static_cast<int>(VdwInteractionType::Count)] = {
    "Cut-off", "Switch", "Shift", "User", "Encad-shift (unused)", "PME", nullptr
};
...
{
    return vdwInteractionType == VdwInteractionType::evdwPME;
}

auto EVDWTYPE(VdwInteractionType vdwInteractionType) -> std::string
{
    return c_vdwInteractionTypeNames[vdwInteractionType];
}

auto evdwNames(VdwInteractionType vdwInteractionType) -> const char*
{
    return EVDWTYPE(vdwInteractionType).c_str();
}
