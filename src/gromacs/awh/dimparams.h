...
struct DimParams
{
     ...
     * \param[in] forceConstant    The harmonic force constant.
     * \param[in] beta             1/(k_B T).
     * \param[in] numLambdaStates  Number of lambda states in the system.
     */
    DimParams(double forceConstant, double beta, int numLambdaStates) :
        k(forceConstant),
        beta(beta),
        betak(beta * forceConstant),
        userCoordUnitsToInternal(1.0),
        numLambdaStates(numLambdaStates)
    {
    }
     ...
     */
    bool isLambdaDimension() const { return numLambdaStates > 0; }
    ...
    const double numLambdaStates;          /**< Number of lambda points in this dimension. */
};
