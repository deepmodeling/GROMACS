 ...
 */
static bool anyDimUsesPull(const std::vector<BiasCoupledToSystem>& biasCoupledToSystem)
{
    for (auto& biasCts : biasCoupledToSystem)
    {
...
Awh::Awh(FILE*                 fplog,
    ...
    pull_(pull_work),
    potentialOffset_(0),
    numLambdaStates_(numLambdaStates),
    lambdaState_(lambdaState)
{
    ...
    for (int k = 0; k < awhParams.numBias; k++)
    {
        ...
        for (int d = 0; d < awhBiasParams.ndim; d++)
        {
            ...
            {
                dimParams.emplace_back(awhDimParams.forceConstant, beta, numLambdaStates_);
            }
        }
                                       ...
                                       FILE*                  fplog)
{
    ...
    for (auto& biasCts : biasCoupledToSystem_)
    {
        ...
        {
            if (!biasCts.bias_.isLambdaDim(d))
            {
        ...
        for (int d = 0; d < biasCts.bias_.ndim(); d++)
        {
            if (!biasCts.bias_.dimParams()[d].isLambdaDimension())
            {
            ...
            else
            {
                ...
                lambdaState_ = biasCts.bias_.getGridCoordValue(umbrellaGridpointIndex)[d];
                numLambdaDimsCounted += 1;
            }
        }
