...
GridAxis::GridAxis(double origin, double end, double period, int numPoints, bool isLambdaAxis) :
    origin_(origin),
    period_(period),
    numPoints_(numPoints),
    isLambdaAxis_(isLambdaAxis)
{
    if (isLambdaAxis)
    {
        length_            = end - origin_;
...
BiasGrid::BiasGrid(const std::vector<DimParams>& dimParams, const AwhDimParams* awhDimParams)
{
    ...
    for (size_t d = 0; d < dimParams.size(); d++)
    {
        ...
        {
            axis_.emplace_back(origin, end, 0, dimParams[d].numLambdaStates, true);
        }
                       ...
                       const std::string&   correctFormatMessage)
{
    ...
    std::vector<bool> isLambdaAxis(grid.numDimensions());
    for (int d = grid.numDimensions() - 1; d >= 0; d--)
    {
        ...
        isLambdaAxis[d] = grid.axis(d).isLambdaAxis();
    }
