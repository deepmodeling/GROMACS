...
#define NFILE asize(fnm)
    ...
    if (bCalcGeom)
    {
        ...
        printf("    box angles  :%7.2f%7.2f%7.2f (degrees)\n",
               norm2(box[ZZ]) == 0 ? 0 : gmx::RAD2DEG * gmx_angle(box[YY], box[ZZ]),
               norm2(box[ZZ]) == 0 ? 0 : gmx::RAD2DEG * gmx_angle(box[XX], box[ZZ]),
               norm2(box[YY]) == 0 ? 0 : gmx::RAD2DEG * gmx_angle(box[XX], box[YY]));
    ...
    if (bScale)
    {
        ...
        if (bRho)
        {
            ...
            vol  = det(box);
            dens = (mass * gmx::AMU) / (vol * gmx::NANO * gmx::NANO * gmx::NANO);
