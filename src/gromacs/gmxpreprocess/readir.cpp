            ...
            warninp_t       wi)
{
    ...
    fep->sc_gapsys          = (get_eeenum(&inp, "sc-gapsys", yesno_names, wi) != 0);
    ...
    if (sits->sits_cal_mode == 0)
    {
        ...
        float temp0 = get_ereal(&inp, "sits-t-ref", 298.0, wi);
        sits->beta0 = 1.0f / CONSTANT_kB / temp0;

        float temph = get_ereal(&inp, "sits-t-high", temp0 * 2.0f, wi);
        float templ = get_ereal(&inp, "sits-t-low", temp0 / 1.2f, wi);
