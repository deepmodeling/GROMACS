...
public:
    ...
    //! Tells whether warnings and/or errors are expected form inputrec parsing and checking and whether we should compare the output
                 ...
                 const TestBehavior testBehavior = TestBehavior::NoErrorAndCompareOutput)
    {
        ...
        get_ir(inputMdpFilename.c_str(), outputMdpFilename.empty() ? nullptr : outputMdpFilename.c_str(),
...
{
    const char* inputMdpFile[] = { "mts = yes", "mts-levels = 2", "mts-level2-factor = 2",
                                   "nstcalcenergy = 5" };
...
{
    const char* inputMdpFile[] = { "mts = yes", "mts-levels = 2", "mts-level2-factor = 2",
                                   "pcoupl = Berendsen", "nstpcouple = 5" };
    runTest(joinStrings(inputMdpFile, "\n"), TestBehavior::ErrorAndDoNotCompareOutput);
}

TEST_F(GetIrTest, MtsCheckNstdhdl)
{
    const char* inputMdpFile[] = { "mts = yes", "mts-level2-factor = 2", "free-energy = yes",
                                   "nstdhdl = 5" };
    runTest(joinStrings(inputMdpFile, "\n"), TestBehavior::ErrorAndDoNotCompareOutput);
}
