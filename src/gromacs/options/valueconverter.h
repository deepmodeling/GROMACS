...
public:
    /*! \brief
     * Converts a std::any value to the output type.
     *
     * \returns  Converted value.
     * \throws InvalidInputError If the input std::any has a type that is
     *     not recognized by any conversion.
     */
    OutType convert(const std::any& value) const
    {
        std::type_index type(value.type());
        auto            iter = converters_.find(type);
        if (iter == converters_.end())
        {
            if (value.type() == typeid(OutType))
            {
                return std::any_cast<OutType>(value);
            }
    ...
    {
        converters_[std::type_index(typeid(InType))] = [func](const std::any& value) {
            return func(std::any_cast<InType>(value));
        };
    }
    /*! \brief
    ...
    {
        converters_[std::type_index(typeid(InType))] = [](const std::any& value) {
            return static_cast<OutType>(std::any_cast<InType>(value));
        };
    }

private:
    typedef std::function<OutType(const std::any& value)> ConversionFunction;
