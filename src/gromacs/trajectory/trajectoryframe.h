...
//! \brief Allocates memory for a trajectory frame for the given number of atoms.
//!
//! The various fields (x, f, v, index) are only allocated if the input frame has corresponding
//! flags bX, bY, bF, and bIndex set to true.
//! \param numAtoms Number of atoms to allocate memory for.
//! \param atoms Optional t_atoms that will be copied into the trxframe.
void initFrame(t_trxframe* frame, int numAtoms, t_atoms* atoms);
//! Copies the contents of one frame to another, without allocation. Assumes the same number of
//! atoms between fields.
