...
void gmx_write_pdb_box(FILE* out, PbcType pbcType, const matrix box)
{
    ...
    {
        alpha = gmx::RAD2DEG * gmx_angle(box[YY], box[ZZ]);
    }
    else
    {
        alpha = 90;
    ...
    {
        beta = gmx::RAD2DEG * gmx_angle(box[XX], box[ZZ]);
    }
    else
    {
        beta = 90;
    ...
    {
        gamma = gmx::RAD2DEG * gmx_angle(box[XX], box[YY]);
    }
    else
    {
        gamma = 90;
...
#define SG_SIZE 11
    ...
    if (box)
    {
        ...
        if ((alpha != 90.0) || (beta != 90.0) || (gamma != 90.0))
        {
            if (alpha != 90.0)
            {
                cosa = std::cos(alpha * gmx::DEG2RAD);
            }
            else
            {
                cosa = 0;
            ...
            if (gamma != 90.0)
            {
                cosg = std::cos(gamma * gmx::DEG2RAD);
                sing = std::sin(gamma * gmx::DEG2RAD);
            }
            else
            {
                cosg = 0;
