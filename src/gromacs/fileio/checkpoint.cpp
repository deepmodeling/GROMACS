...
static int do_cpte_matrix(XDR* xd, StatePart part, int ecpt, int sflags, matrix v, FILE* list)
{
    ...
    ret = doVectorLow<real, std::allocator<real>>(
            xd, part, ecpt, sflags, gmx::c_dim * gmx::c_dim, nullptr, &vr, nullptr, nullptr, CptElementType::matrix3x3);
...
static int do_cpte_matrices(XDR* xd, StatePart part, int ecpt, int sflags, int n, matrix** v, FILE* list)
{
    ...
    ret = doVectorLow<real, std::allocator<real>>(
            xd, part, ecpt, sflags, nf * gmx::c_dim * gmx::c_dim, nullptr, &vr, nullptr, nullptr, CptElementType::matrix3x3);
    for (i = 0; i < nf; i++)
    {
        for (j = 0; j < gmx::c_dim; j++)
        {
