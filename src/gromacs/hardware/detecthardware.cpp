                            ...
                            compat::not_null<gmx_hw_info_t*> hardwareInfo)
{
    ...
    if (!DevicesManager::canPerformDeviceDetection(&errorMessage))
    {
        return;
    }
    ...
    {
        std::string errorMessage;
        gpusCanBeDetected = DevicesManager::isDeviceDetectionFunctional(&errorMessage);
    ...
    if (!allRanksMustDetectGpus && !deviceInfos.empty())
    {
        ...
        DevicesManager::serializeDevicesInformation(hardwareInfo->deviceInfos, &writer);
        ...
        hardwareInfo->deviceInfos = DevicesManager::deserializeDeviceInfos(&writer);
#endif
    }
    ...
    static void hardwareTopologyDoubleCheckDetection(
            const gmx::MDLogger gmx_unused& mdlog, const gmx::HardwareTopology gmx_unused& hardwareTopology)
    {
#if defined HAVE_SYSCONF && defined(_SC_NPROCESSORS_CONF)
                                       ...
                                       const PhysicalNodeCommunicator& physicalNodeComm)
    {
        ...
        gmx_collect_hardware_mpi(*hardwareInfo->cpuInfo, physicalNodeComm,
