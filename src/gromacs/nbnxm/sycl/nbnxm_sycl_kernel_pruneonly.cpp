    ...
    return [=](cl::sycl::nd_item<1> itemIdx) [[intel::reqd_sub_group_size(requiredSubGroupSize)]]
    {
        ...
        if (tidxz == 0)
        {
            for (int i = 0; i < c_nbnxnGpuNumClusterPerSupercluster; i += c_clSize)
            {
                ...
                const gmx::float4 xq    = a_xq[ai];
                const gmx::float3 shift = a_shiftVec[nbSci.shift];
                const gmx::float4 xi(xq[0] + shift[0], xq[1] + shift[1], xq[2] + shift[2], xq[3]);
            ...
            if (imaskCheck)
            {
                for (int jm = 0; jm < c_nbnxnGpuJgroupSize; jm++)
                {
                    if (imaskCheck & (superClInteractionMask << (jm * c_nbnxnGpuNumClusterPerSupercluster)))
                    {
                        ...
                        /* load j atom data */
                        const gmx::float4 tmp = a_xq[aj];
                        const gmx::float3 xj(tmp[0], tmp[1], tmp[2]);
                        ...
                        for (int i = 0; i < c_nbnxnGpuNumClusterPerSupercluster; i++)
                        {
                            if (imaskCheck & mask_ji)
                            {
                                ...
                                const gmx::float4 xi = sm_xq[i][tidxi];
                                // distance between i and j atoms
                                gmx::float3 rv(xi[0], xi[1], xi[2]);
                                rv -= xj;
