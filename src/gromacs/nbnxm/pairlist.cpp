...
static constexpr bool isCpuPairlistType(PairlistType pairlistType)
{
    return (pairlistType == PairlistType::Simple4x2 || pairlistType == PairlistType::Simple4x4
            || pairlistType == PairlistType::Simple4x8);
}

static constexpr bool isGpuPairlistType(PairlistType pairlistType)
{
    return !isCpuPairlistType(pairlistType);
}
...
PairlistSet::PairlistSet(const InteractionLocality locality, const PairlistParams& pairlistParams) :
    ...
    combineLists_(isGpuPairlistType(pairlistParams.pairlistType)), // Currently GPU lists are always combined
    isCpuType_(isCpuPairlistType(pairlistParams.pairlistType))
{
...
#pragma omp parallel for num_threads(numLists) schedule(static) shared(fepLists_, numLists) default(none)
    ...
    while (next_ci(iGrid, nth, ci_block, &ci_x, &ci_y, &ci_b, &ci))
    {
        ...
        for (int tz = -shp[ZZ]; tz <= shp[ZZ]; tz++)
        {
            ...
            for (int ty = -shp[YY]; ty <= shp[YY]; ty++)
            {
                ...
                real by0 = bSimple ? bb_i[ci].lower.y + shy
                                   : iGridDims.lowerCorner[YY] + (real(ci_y)) * iGridDims.cellSize[YY] + shy;
                real by1 = bSimple ? bb_i[ci].upper.y + shy
                ...
                for (int tx = -shp[XX]; tx <= shp[XX]; tx++)
                {
                    ...
                    real bx0 = bSimple ? bb_i[ci].lower.x + shx
                                       : iGridDims.lowerCorner[XX]
                                                 + (real(ci_x)) * iGridDims.cellSize[XX] + shx;
                    real bx1 = bSimple ? bb_i[ci].upper.x + shx
                                       : iGridDims.lowerCorner[XX]
