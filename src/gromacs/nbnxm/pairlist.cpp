...
#define PRUNE_LIST_CPU_ALL 0
    ...
    for (int subc = 0; subc < jGrid.numClustersPerCell()[scj]; subc++)
    {
        ...
        const int ci1 = (excludeSubDiagonal && sci == scj) ? subc + 1 : iGrid.numClustersPerCell()[sci];
                          ...
                          t_nblist*       nlist)
{
    ...
    for (int i = 0; i < nbl->na_ci; i++)
    {
        ...
        if (ai >= 0)
        {
            ...
            const bool bFEP_i = iGrid.atomIsPerturbed(ci - iGrid.cellOffset(), i);
    ...
    for (int c = 0; c < c_gpuNumClusterPerCell; c++)
    {
        ...
        for (int i = 0; i < nbl->na_ci; i++)
        {
            ...
            if (ai >= 0)
            {
                ...
                const real xi = nbat->x()[ind_i * nbat->xstride + XX] + shx;
                const real yi = nbat->x()[ind_i * nbat->xstride + YY] + shy;
                const real zi = nbat->x()[ind_i * nbat->xstride + ZZ] + shz;
                        ...
                        if (bFEP_i || jGrid.clusterIsPerturbed(cjr))
                        {
                            for (int j = 0; j < nbl->na_cj; j++)
                            {
                                    ...
                                    && (!bDiagRemoved || ind_j >= ind_i))
                                {
                                    ...
                                    const int          excl_pair = a_mod_wj(j) * nbl->na_ci + i;
                                    const unsigned int excl_bit =
                                            (1U << (gcj * c_gpuNumClusterPerCell + c));

                                    const real dx = nbat->x()[ind_j * nbat->xstride + XX] - xi;
                                    const real dy = nbat->x()[ind_j * nbat->xstride + YY] - yi;
                                    const real dz = nbat->x()[ind_j * nbat->xstride + ZZ] - zi;
...
#pragma omp parallel for schedule(static) num_threads(numLists)
    ...
    for (int th = 0; th < numLists; th++)
    {
        ...
        for (int i = 0; i < nbls->nri; i++)
        {
            ...
            int nrj = nbls->jindex[i + 1] - nbls->jindex[i];
                                     ...
                                     t_nblist*               nbl_fep)
{
    ...
    int na_cj_2log = get_2log(nbl->na_cj);
    ...
    float rbb2 = boundingbox_only_distance2(iGridDims, jGridDims, nbl->rlist, pairlistIsSimple(*nbl));
    ...
    while (next_ci(iGrid, nth, ci_block, &ci_x, &ci_y, &ci_b, &ci))
    {
        ...
        const int ci_xy = ci_x * iGridDims.numCells[YY] + ci_y;
            ...
            for (int ty = -shp[YY]; ty <= shp[YY]; ty++)
            {
                ...
                for (int tx = -shp[XX]; tx <= shp[XX]; tx++)
                {
                    ...
                    for (int cx = cxf; cx <= cxl; cx++)
                    {
                         ...
                         */
                        const int cyf_x = (isIntraGridList && cx == 0
