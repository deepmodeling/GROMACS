...
#    else
    ...
    const real gmx_unused tab_coul_V = ic->coulombEwaldTables->tableV.data();
#    endif
#endif
    ...
    for (const nbnxn_ci_t& ciEntry : nbl->ci)
    {
         ...
         */
        bool do_LJ   = ((ciEntry.shift & NBNXN_CI_DO_LJ(0)) != 0);
        bool do_coul = ((ciEntry.shift & NBNXN_CI_DO_COUL(0)) != 0);
        bool half_LJ = (((ciEntry.shift & NBNXN_CI_HALF_LJ(0)) != 0) || !do_LJ) && do_coul;
#ifdef CALC_ENERGIES
