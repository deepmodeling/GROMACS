                          ...
                          real*                      Vvdw)
{
    ...
    bool bEwald = EEL_FULL(iconst->eeltype);
    if (bEwald)
    {
    ...
    real rlist2 = nbl->rlist * nbl->rlist;
        ...
        if (nbln.shift == CENTRAL && nbl->cj4[cj4_ind0].cj[0] == sci * c_nbnxnGpuNumClusterPerSupercluster)
        {
            ...
            for (int im = 0; im < c_nbnxnGpuNumClusterPerSupercluster; im++)
            {
                ...
                for (int ic = 0; ic < c_clSize; ic++)
                {
                    ...
                    const real iq = x[ia * nbat->xstride + 3];
                    vctot += iq * iq;
                }
            }
            if (!bEwald)
                        ...
                        & 1)
                    {
                        ...
                        for (int ic = 0; ic < c_clSize; ic++)
                        {
                            ...
                            for (int jc = 0; jc < c_clSize; jc++)
                            {
                                ...
                                const int  js  = ja * nbat->xstride;
                                const int  jfs = ja * nbat->fstride;
