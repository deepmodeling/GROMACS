                          ...
                          real*                      Vvdw)
{
    ...
    for (const nbnxn_sci_t& nbln : nbl->sci)
    {
        ...
        if (nbln.shift == CENTRAL && nbl->cj4[cj4_ind0].cj[0] == sci * c_nbnxnGpuNumClusterPerSupercluster)
        {
            ...
            if (!bEwald)
            {
                vctot *= -facel * 0.5 * iconst->ReactionFieldCoefficients->correction_;
            }
            else
            {
                        ...
                        & 1)
                    {
                        ...
                        for (ic = 0; ic < c_clSize; ic++)
                        {
                            ...
                            for (jc = 0; jc < c_clSize; jc++)
                            {
                                ...
                                {
                                    /* Reaction-field */
                                    krsq  = iconst->ReactionFieldCoefficients->constant_ * rsq;
                                    fscal = qq * (int_bit * rinv - 2 * krsq) * rinvsq;
                                    if (stepWork.computeEnergy)
                                    {
                                        vcoul = qq
                                                * (int_bit * rinv + krsq
                                                   - iconst->ReactionFieldCoefficients->correction_);
                                    }
                                }
                                else
                                {
