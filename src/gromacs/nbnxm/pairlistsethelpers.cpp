 ...
 */
static float clusterBoundingBoxDistance2(const Nbnxm::BoundingBox& bb_i, const Nbnxm::BoundingBox& bb_j)
{
...
void print_nblist_statistics(FILE* fp, const NbnxnPairlistCpu& nbl, const Nbnxm::GridSet& gridSet, const real rl)
{
...
void print_nblist_statistics(FILE* fp, const NbnxnPairlistGpu& nbl, const Nbnxm::GridSet& gridSet, const real rl)
{
    const Grid&             grid = gridSet.grids()[0];
    const Grid::Dimensions& dims = grid.dimensions();
...
{
    const Nbnxm::BoundingBox* gmx_restrict bb_ci = nbl->work->iClusterData.bb.data();
...
#else
    const Nbnxm::BoundingBox* bb_ci = work.iSuperClusterData.bb.data();
#endif
...
static inline void set_icell_bb_simple(gmx::ArrayRef<const Nbnxm::BoundingBox> bb,
                                       int                                     ci,
                                       real                                    shx,
                                       real                                    shy,
                                       real                                    shz,
                                       Nbnxm::BoundingBox*                     bb_ci)
{
...
/* Sets a super-cell and sub cell bounding boxes, including PBC shift */
gmx_unused static void set_icell_bb_supersub(gmx::ArrayRef<const Nbnxm::BoundingBox> bb,
                                             int                                     ci,
                                             real                                    shx,
                                             real                                    shy,
                                             real                                    shz,
                         ...
                         float*                         nsubpair_tot_est)
{
    ...
    if (iloc == gmx::InteractionLocality::Local)
    {
...
void balance_fep_lists(gmx::ArrayRef<std::unique_ptr<t_nblist>> fepLists, gmx::ArrayRef<PairsearchWork> work)
{
...
int get_ci_block_size(const Grid& iGrid, const bool haveMultipleDomains, const int numLists)
{
                              ...
                              t_nblist*               nbl_fep)
{
    ...
    gmx::ArrayRef<const Nbnxm::BoundingBox> bb_i;
#if NBNXN_BBXXXX
    ...
    bb_i = iGrid.iBoundingBoxes();
#endif
    gmx::ArrayRef<const Nbnxm::BoundingBox1D> bbcz_i  = iGrid.zBoundingBoxes();
    gmx::ArrayRef<const int>                  flags_i = iGrid.clusterFlags();
    gmx::ArrayRef<const Nbnxm::BoundingBox1D> bbcz_j  = jGrid.zBoundingBoxes();
...
template void nbnxn_make_pairlist_part<NbnxnPairlistCpu>(const Nbnxm::GridSet&        gridSet,
...
template void nbnxn_make_pairlist_part<NbnxnPairlistGpu>(const Nbnxm::GridSet&        gridSet,
                                                         const Nbnxm::Grid&           iGrid,
                                                         const Nbnxm::Grid&           jGrid,
                                                         PairsearchWork*              work,
                                                         const nbnxn_atomdata_t*      nbat,
...
Range<int> getIZoneRange(const Nbnxm::GridSet::DomainSetup& domainSetup, const gmx::InteractionLocality locality)
{
    ...
    else if (locality == gmx::InteractionLocality::Local)
    {
...
Range<int> getJZoneRange(const gmx_domdec_zones_t*      ddZones,
                         const gmx::InteractionLocality locality,
                         const int                      iZone)
{
    if (locality == gmx::InteractionLocality::Local)
    {
