...
static std::tuple<real, real> getTargetCellLength(const Grid::Geometry& geometry, const real atomDensity)
{
    if (geometry.isSimple)
    {
        ...
        return std::make_tuple(tlen, tlen);
    }
    else
    {
        /* Approximately cubic sub cells */
        real tlen = std::cbrt(geometry.numAtomsICluster / atomDensity);
        return std::make_tuple(tlen * c_gpuNumClusterPerCellX, tlen * c_gpuNumClusterPerCellY);
    }
}
                         ...
                         gmx::PinningPolicy pinningPolicy)
{
    ...
    if (numAtoms > geometry_.numAtomsPerCell)
    {
        ...
        /* target cell length */
        const auto [tlen_x, tlen_y] = getTargetCellLength(geometry_, atomDensity);
