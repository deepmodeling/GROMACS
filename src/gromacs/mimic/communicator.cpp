...
void gmx::MimicCommunicator::sendInitData(gmx_mtop_t* mtop, PaddedHostVector<gmx::RVec> coords)
{
    ...
    {
        convertedCoords.push_back(static_cast<double>(coord[0]) / gmx::BOHR2NM);
        convertedCoords.push_back(static_cast<double>(coord[1]) / gmx::BOHR2NM);
        convertedCoords.push_back(static_cast<double>(coord[2]) / gmx::BOHR2NM);
    }
...
void gmx::MimicCommunicator::getCoords(PaddedHostVector<RVec>* x, const int natoms)
{
    ...
    {
        (*x)[j][0] = static_cast<real>(coords[j * 3] * gmx::BOHR2NM);
        (*x)[j][1] = static_cast<real>(coords[j * 3 + 1] * gmx::BOHR2NM);
        (*x)[j][2] = static_cast<real>(coords[j * 3 + 2] * gmx::BOHR2NM);
    }
}

void gmx::MimicCommunicator::sendEnergies(real energy)
{
    double convertedEnergy = energy / (gmx::HARTREE2KJ * gmx::AVOGADRO);
...
void gmx::MimicCommunicator::sendForces(gmx::ArrayRef<gmx::RVec> forces, int natoms)
{
    ...
    for (int j = 0; j < natoms; ++j)
    {
        convertedForce.push_back(static_cast<real>(forces[j][0]) / gmx::HARTREE_BOHR2MD);
        convertedForce.push_back(static_cast<real>(forces[j][1]) / gmx::HARTREE_BOHR2MD);
        convertedForce.push_back(static_cast<real>(forces[j][2]) / gmx::HARTREE_BOHR2MD);
    }
