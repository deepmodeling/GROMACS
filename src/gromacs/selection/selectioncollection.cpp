...
SelectionCollection::SelectionCollection(const SelectionCollection& rhs) : impl_(new Impl)
{
    ...
    for (size_t i = 0; i < rhs.impl_->sc_.sel.size(); i++)
    {
        ...
        // Each parse adds an element to sc_.sel, then set the flags
        impl_->sc_.sel[i]->setFlags(selectionOption->flags());
    }

    // Topology has been initialized if top is non-null or natoms is set.
