...
public:
    ...
    // Creates a copy of the selection collection and replaces the sel_ with that copy.
    // Used for validating copy constructor/assignment operator.
    void makeCopy();
...
void SelectionCollectionTest::makeCopy()
{
...
TEST_F(SelectionCollectionTest, retrieveValidSelection)
{
    ...
    ASSERT_NE(retrievedSel, std::nullopt);
    ASSERT_STREQ(sel_[0].name(), retrievedSel->name());
}

TEST_F(SelectionCollectionTest, retrieveInvalidSelection)
{
    ASSERT_EQ(sc_.selection("some invalid key"), std::nullopt);
}

/********************************************************************
 * Tests for assignment/copying.
 */
TEST_F(SelectionCollectionTest, copyEmptyCollection)
{
    makeCopy();
}

TEST_F(SelectionCollectionTest, copiedSelectionListsHandledSeparately)
{
    ASSERT_NO_THROW_GMX(sel_ = sc_.parseFromString("atomnr 1 to 10; none"));
    EXPECT_FALSE(sc_.requiredTopologyProperties().hasAny());
    ASSERT_NO_FATAL_FAILURE(setAtomCount(10));
...
TEST_F(SelectionCollectionDataTest, CopiedSelectionWorksPreCompilation)
{
    ...
    for (const std::string_view selName : selNames)
    {
        ...
        ASSERT_NE(maybeSel, std::nullopt);
