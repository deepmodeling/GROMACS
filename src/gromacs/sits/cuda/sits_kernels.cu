...
void SITS::Initial_SITS(CONTROLLER* controller, int atom_numbers)
{
    ...
    else if (info.sits_mode == CLASSICAL_SITS_MODE)
    {
        ...
        if (!classical_info.constant_nk)
        {
            ...
            {
                classical_info.record_interval =
            ...
            {
                classical_info.update_interval =
        ...
        Malloc_Safely((void**)&classical_info.log_norm_recorded_cpu,
                      sizeof(float) * classical_info.k_numbers);
        if (!classical_info.constant_nk)
        {
            // nk的轨迹文件
            ...
            if (controller[0].Command_Exist("sits_norm_traj_file"))
            {
                ...
                Open_File_Safely(&classical_info.norm_traj_file,
        ...
        cudaMemcpy(classical_info.log_nk, tempf, sizeof(float) * classical_info.k_numbers,
        ...
        cudaMemcpy(classical_info.log_norm, tempf, sizeof(float) * classical_info.k_numbers,
                   cudaMemcpyHostToDevice);
        cudaMemcpy(classical_info.log_norm_old, tempf, sizeof(float) * classical_info.k_numbers,
                   cudaMemcpyHostToDevice);
        ...
        cudaMemcpy(classical_info.beta_k, tempf, sizeof(float) * classical_info.k_numbers,
                   cudaMemcpyHostToDevice);
