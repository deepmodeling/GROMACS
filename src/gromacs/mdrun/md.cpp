...
void gmx::LegacySimulator::do_md()
{
    ...
    while (!bLastStep)
    {
        ...
        {
            bool bIfRandomized = update_randomize_velocities(ir, step, cr, mdatoms, state->v, &upd,
                                                             constr != nullptr);
            if (bIfRandomized)
            {
                constrain_velocities(step, nullptr, state, nullptr, constr, false, do_log, do_ene);
            }
        }
