...
static int imd_send_rvecs(IMDSocket* socket, int nat, rvec* x, char* buffer)
{
    ...
    for (i = 0; i < nat; i++)
    {
        sendx[0] = static_cast<float>(x[i][0]) * gmx::NM2A;
        sendx[1] = static_cast<float>(x[i][1]) * gmx::NM2A;
        sendx[2] = static_cast<float>(x[i][2]) * gmx::NM2A;
...
{
    int  i;
    real conversion = gmx::CAL2JOULE * gmx::NM2A;
