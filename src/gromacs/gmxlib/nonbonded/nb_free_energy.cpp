...
#define NSTATES 2
    ...
    for (int n = 0; n < nri; n++)
    {
        ...
        for (int k = nj0; k < nj1; k += DataTypes::simdRealWidth)
        {
            ...
            const BoolType computeVdwEwaldInteraction = (bPairNotIncluded || r < rVdw);
...
#pragma omp atomic
    ...
    {
#pragma omp atomic
        dvdl[static_cast<int>(FreeEnergyPerturbationCouplingType::Coul)] += gmx::reduce(dvdlCoul);
    }
    if (gmx::anyTrue(dvdlVdw != zero))
    {
#pragma omp atomic
        dvdl[static_cast<int>(FreeEnergyPerturbationCouplingType::Vdw)] += gmx::reduce(dvdlVdw);
    }
