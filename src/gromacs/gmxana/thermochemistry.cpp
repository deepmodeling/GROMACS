...
{
    double factor_gmx_to_omega2 = 1.0E21 / (gmx::AVOGADRO * gmx::AMU);
...
double calcZeroPointEnergy(gmx::ArrayRef<const real> eigval, real scale_factor)
{
    ...
    double factor = gmx::PLANCK * gmx::PICO / (2.0 * M_PI);
    double zpe    = 0;
    for (auto& r : eigval)
    {
...
double calcVibrationalInternalEnergy(gmx::ArrayRef<const real> eigval, real temperature, gmx_bool linear, real scale_factor)
{
    ...
    return temperature * gmx::BOLTZ * Evib;
}
...
{
    double kT = gmx::BOLTZ * temperature;
    ...
    double qT = (std::pow(2 * M_PI * mass * kT / gmx::square(gmx::PLANCK), 1.5) * (kT / P)
                 * (1e30 / gmx::AVOGADRO));
    return gmx::RGAS * (std::log(qT) + 2.5);
}
...
{
    size_t nskip  = bLinear ? 5 : 6;
    double hbar   = gmx::PLANCK1 / (2 * M_PI);          // J s
    double kt     = gmx::BOLTZMANN * temperature;       // J
    double kteh   = kt * std::exp(2.0) / (hbar * hbar); // 1/(J s^2) = 1/(kg m^2)
    double evcorr = gmx::NANO * gmx::NANO * gmx::AMU;
    if (debug)
    {
