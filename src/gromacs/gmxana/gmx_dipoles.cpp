...
#define e2d(x) gmx::ENM2DEBYE*(x)
constexpr double EANG2CM = gmx::E_CHARGE * 1.0e-10;       /* e Angstrom to Coulomb meter */
constexpr double CM2D    = gmx::SPEED_OF_LIGHT * 1.0e+24; /* Coulomb meter to Debye */
...
static void print_gkrbin(const char* fn, t_gkrbin* gb, int ngrp, int nframes, real volume, const gmx_output_env_t* oenv)
{
    ...
    for (i = 0; i < last; i++)
    {
        ...
        ener = -0.5 * cosav * gmx::ONE_4PI_EPS0 / (x1 * x1 * x1);
...
static real calc_eps(double M_diff, double volume, double epsRF, double temp)
{
    ...
    A = M_diff * fac / (3 * eps_0 * volume * gmx::NANO * gmx::NANO * gmx::NANO * gmx::BOLTZMANN * temp);
