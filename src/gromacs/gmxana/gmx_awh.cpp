                                         ...
                                         real                 kTValue)
{
    ...
    if (graphSelection == AwhGraphSelection::All)
    {
        yLabel_ += gmx::formatString(", (nm\\S-%d\\N, rad\\S-%d\\N or lambda state\\S-%d\\N), (-)",
                                     awhBiasParams->ndim, awhBiasParams->ndim, awhBiasParams->ndim);
    }
}
                                              ...
                                              real                 kTValue)
{
    ...
    if (useKTForEnergy_)
    {
        yLabel_ = "friction/k\\sB\\NT (nm\\S-2\\Nps, rad\\S-2\\Nps or lambda state\\S-2\\Nps)";
    }
    else
    {
        yLabel_ =
                "friction (kJ mol\\S-1\\Nnm\\S-2\\Nps, kJ mol\\S-1\\Nrad\\S-2\\Nps or kJ "
                "mol\\S-1\\Nlambda state\\S-2\\Nps)";
    }
}
