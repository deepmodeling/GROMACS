...
static double calc_Shs(double f, double y)
{
    ...
    return gmx::BOLTZ * (std::log(calc_compress(fy)) + fy * (3 * fy - 4) / gmx::square(1 - fy));
}

static real wCsolid(real nu, real beta)
{
    real bhn = beta * gmx::PLANCK * nu;
    real ebn, koko;
...
#define NFILE asize(fnm)
    ...
    Delta = ((2 * DoS0 / (9 * Natom)) * std::sqrt(M_PI * gmx::BOLTZ * Temp * Natom / tmass)
    ...
    Sig   = gmx::BOLTZ
          * (5.0 / 2.0
             + std::log(2 * M_PI * gmx::BOLTZ * Temp / (gmx::square(gmx::PLANCK)) * V / (f * Natom)));
    Shs   = Sig + calc_Shs(f, y);
    rho   = (tmass * gmx::AMU) / (V * gmx::NANO * gmx::NANO * gmx::NANO);
    ...
    recip_fac = bRecip ? (1e7 / gmx::SPEED_OF_LIGHT) : 1.0;
    for (j = 0; (j < nframes / 4); j++)
    {
    ...
    wCdiff = 0.5;
    wSdiff = Shs / (3 * gmx::BOLTZ); /* Is this correct? */
    wEdiff = 0.5;
    ...
    cP = gmx::BOLTZ * evaluate_integral(nframes / 4, nu, dos[DOS_CP], nullptr, int{ nframes / 4 }, &stddev);
