...
#define NFILE asize(fnm)
    ...
    do
    {
        ...
        for (i = 0; i < bun.n; i++)
        {
            ...
            fprintf(ftilt, " %6g", gmx::RAD2DEG * acos(bun.dir[i][ZZ]));
            comp = bun.mid[i][XX] * bun.dir[i][XX] + bun.mid[i][YY] * bun.dir[i][YY];
            fprintf(ftiltr, " %6g", gmx::RAD2DEG * std::asin(comp / std::hypot(comp, bun.dir[i][ZZ])));
            comp = bun.mid[i][YY] * bun.dir[i][XX] - bun.mid[i][XX] * bun.dir[i][YY];
            fprintf(ftiltl, " %6g", gmx::RAD2DEG * std::asin(comp / std::hypot(comp, bun.dir[i][ZZ])));
            if (bKink)
            {
                ...
                unitv(va, va);
                unitv(vb, vb);
                fprintf(fkink, " %6g", gmx::RAD2DEG * acos(iprod(va, vb)));
                cprod(va, vb, vc);
                copy_rvec(bun.mid[i], vr);
                vr[ZZ] = 0;
                unitv(vr, vr);
                fprintf(fkinkr, " %6g", gmx::RAD2DEG * std::asin(iprod(vc, vr)));
                vl[XX] = vr[YY];
                vl[YY] = -vr[XX];
                vl[ZZ] = 0;
                fprintf(fkinkl, " %6g", gmx::RAD2DEG * std::asin(iprod(vc, vl)));
            }
        }
        fprintf(flen, "\n");
        fprintf(fdist, "\n");
