...
#define NFILE asize(fnm)
    ...
    if (bAver)
    {
        ...
        for (i = 0; (i < nframes); i++)
        {
            fprintf(out, "%10.5f  %8.3f", time[i], aver_angle[i] * gmx::RAD2DEG);
            if (bALL)
            {
                for (j = 0; (j < nangles); j++)
                {
                    ...
                    {
                        real dd = dih[j][i];
                        fprintf(out, "  %8.3f", std::atan2(std::sin(dd), std::cos(dd)) * gmx::RAD2DEG);
                    }
                    else
                    {
                        fprintf(out, "  %8.3f", dih[j][i] * gmx::RAD2DEG);
                    }
                }
            }
            fprintf(out, "\n");
    ...
    if (bCorr)
    {
        ...
        else
        {
            ...
            if (bChandler)
            {
                real     dval, sixty = gmx::DEG2RAD * 60;
                gmx_bool bTest;
