...
#define INDEX(ppp) (((static_cast<int>(360 + (ppp)*gmx::RAD2DEG)) % 360) / 6)
    x = INDEX(phi);
    y = INDEX(psi);
#undef INDEX
                    ...
                    const gmx_output_env_t* oenv)
{
    ...
    for (i = 0; (i < nlist); i++)
    {
        if ((has_dihedral(edPhi, &(dlist[i]))) && (has_dihedral(edPsi, &(dlist[i]))))
        {
            ...
            for (j = 0; (j < nf); j++)
            {
                phi = gmx::RAD2DEG * dih[Phi][j];
                psi = gmx::RAD2DEG * dih[Psi][j];
                fprintf(fp, "%10g  %10g\n", phi, psi);
                if (bViol)
                {
                    fprintf(gp,
                            "%d\n",
                            static_cast<int>(!bAllowed(dih[Phi][j], gmx::RAD2DEG * dih[Psi][j])));
                }
                if (bOm)
                {
                    omega = gmx::RAD2DEG * dih[Om][j];
        ...
        if ((has_dihedral(edChi1, &(dlist[i]))) && (has_dihedral(edChi2, &(dlist[i]))))
        {
            ...
            for (j = 0; (j < nf); j++)
            {
                fprintf(fp, "%10g  %10g\n", gmx::RAD2DEG * dih[Xi1][j], gmx::RAD2DEG * dih[Xi2][j]);
            }
            xvgrclose(fp);
        }
        else
