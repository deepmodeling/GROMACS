...
static real rot(rvec x1, const rvec x2)
{
    ...
    dphi = gmx::RAD2DEG * std::atan2(yy, xx);
...
void calc_hxprops(int nres, t_bb bb[], const rvec x[])
{
    ...
    for (i = 0; (i < nres); i++)
    {
        ...
        bb[i].jcaha += 1.4 * std::sin((bb[i].psi + 138.0) * gmx::DEG2RAD)
                       - 4.1 * std::cos(2.0 * gmx::DEG2RAD * (bb[i].psi + 138.0))
                       + 2.0 * std::cos(2.0 * gmx::DEG2RAD * (bb[i].phi + 30.0));
    }
}
