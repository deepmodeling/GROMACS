...
{
    double x  = gmx::PLANCK * nu / (gmx::BOLTZ * T);
    double ex = std::exp(x);

    if (nu <= 0)
    {
        return gmx::BOLTZ * gmx::KILO;
    }
    else
    {
        return gmx::BOLTZ * gmx::KILO * (ex * gmx::square(x) / gmx::square(ex - 1) - 1);
    }
}
...
static double u_corr(double nu, double T)
{
    ...
    {
        return gmx::BOLTZ * T * (0.5 * x - 1 + x / (ex - 1));
    }
}
                                   ...
                                   real                     linear_toler)
{
    ...
    double rot_const = gmx::square(gmx::PLANCK) / (8 * gmx::square(M_PI) * gmx::BOLTZ);
...
#define NFILE asize(fnm)
     ...
     */
    factor_gmx_to_omega2       = 1.0E21 / (gmx::AVOGADRO * gmx::AMU);
    factor_omega_to_wavenumber = 1.0E-5 / (2.0 * M_PI * gmx::SPEED_OF_LIGHT);
