                           ...
                           const gmx_output_env_t* oenv)
{
    ...
    for (i = 0; i < nf4; i++)
    {
        ...
        fac = (V * gmx::NANO * gmx::NANO * gmx::NANO * gmx::PICO * 1e10) / (2 * gmx::BOLTZMANN * T)
              / (nint - i);
                                   ...
                                   int         nbmax)
{
    ...
    NANO3 = gmx::NANO * gmx::NANO * gmx::NANO;
    if (!bDriftCorr)
    {
        fprintf(fp,
...
if ((ii[eVol] < nset) && (ii[eTemp] < nset))
{
    ...
    kappa = (varv / vv) / (gmx::BOLTZMANN * tt);
}
/* Enthalpy */
hh = varh = NOTSET;
if ((ii[eEnth] < nset) && (ii[eTemp] < nset))
{
    hh   = gmx::KILO * edat->s[ii[eEnth]].av / gmx::AVOGADRO;
    varh = gmx::square(gmx::KILO * edat->s[ii[eEnth]].rmsd / gmx::AVOGADRO);
    cp   = gmx::AVOGADRO * ((varh / nmol) / (gmx::BOLTZMANN * tt * tt));
}
/* Total energy */
if ((ii[eEtot] < nset) && (hh == NOTSET) && (tt != NOTSET))
{
    ...
    cv    = gmx::KILO * ((varet / nmol) / (gmx::BOLTZ * tt * tt));
}
/* Alpha, dcp */
if ((ii[eVol] < nset) && (ii[eEnth] < nset) && (ii[eTemp] < nset))
{
    ...
    for (j = 0; (j < edat->nframes); j++)
    {
        ...
        h = gmx::KILO * edat->s[ii[eEnth]].ener[j] / gmx::AVOGADRO;
        v_sum += v;
        h_sum += h;
        vh_sum += (v * h);
    }
    ...
    alpha   = (vh_aver - v_aver * h_aver) / (v_aver * gmx::BOLTZMANN * tt * tt);
    dcp     = (v_aver * gmx::AVOGADRO / nmol) * tt * gmx::square(alpha) / (kappa);
}
...
if (tt != NOTSET)
{
    ...
    if (debug != nullptr)
    {
        if (varv != NOTSET)
        {
            fprintf(fp, "varv  =  %10g (m^6)\n", varv * gmx::AVOGADRO / nmol);
        }
    }
    if (vv != NOTSET)
    {
        fprintf(fp, "Volume                                   = %10g m^3/mol\n", vv * gmx::AVOGADRO / nmol);
    }
    if (varh != NOTSET)
    {
        fprintf(fp,
                "Enthalpy                                 = %10g kJ/mol\n",
                hh * gmx::AVOGADRO / (gmx::KILO * nmol));
    }
    if (alpha != NOTSET)
    {
                         ...
                         const gmx_output_env_t* oenv)
{
    ...
    else
    {
        ...
        if (bVisco)
        {
            ...
            factor = (Vaver * 1e-26 / (gmx::BOLTZMANN * Temp)) * Dt;
                ...
                const gmx_output_env_t* oenv)
{
    ...
    for (i = 0; i < nset; i++)
    {
        ...
        for (j = 0; j < nenergy; j++)
        {
            ...
            if (fp)
            {
                fprintf(fp, "%10g %10g %10g\n", time[j], dE, -gmx::BOLTZ * reftemp * std::log(sum / (j + 1)));
            }
        }
        aver = -gmx::BOLTZ * reftemp * std::log(sum / nenergy);
