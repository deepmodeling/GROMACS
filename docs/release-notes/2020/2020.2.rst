...
Reduce time spend in grompp for large numbers of distance restraints
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

:ref:`gmx grompp` would spend hours for processing large numbers of distance restraints
due to an issue with the computational cost scaling quadratically.
